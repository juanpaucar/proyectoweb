...
class Sorteo{
  ...
  public function createSorteo(){
    ...
    $sql_query = "SELECT codigo FROM token ORDER BY RAND() LIMIT 1;";
    ...
    if ($result->num_rows > 0){
      ...
      if ($connection->query($sql_query)) {
        $sql_query = "SELECT user.name FROM user join token on user.id=token.user_id where token.codigo='$codigo';";
        $result = $connection->query($sql_query);
        if ($result->num_rows > 0) {
          $data = $result->fetch_assoc();
          $winner = $data['name'];
